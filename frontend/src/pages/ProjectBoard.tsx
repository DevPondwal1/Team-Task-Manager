...
import { ArrowLeft, Plus, Calendar, User, MoreHorizontal, Loader2 } from 'lucide-react';
                  ...
                  <div key={task.id} className="glass-panel p-4 cursor-grab active:cursor-grabbing hover:border-primary/30 transition-colors group">
                    <div className="flex justify-between items-start mb-2">
                      ...
                      <div className="relative group/menu">
                        ...
                        <div className="absolute right-0 mt-1 w-36 bg-surface border border-surfaceHighlight rounded-lg shadow-xl opacity-0 invisible group-hover/menu:opacity-100 group-hover/menu:visible transition-all z-10 py-1">
                          {columns.map(c => c.id !== task.status && (
                            <button 
                              key={c.id} 
                              onClick={() => handleStatusChange(task.id, c.id)}
                              className="w-full text-left px-3 py-1.5 text-sm text-text-muted hover:bg-surfaceHighlight hover:text-white transition-colors"
                            >
                              Move to {c.title}
                            </button>
                          ))}
